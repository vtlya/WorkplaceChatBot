                ...
                if message.get('postback'):
                    ...
                    if message['message'].get('attachments'):
                        ...
                        send_message(recipient_id,
                                     'К сожалению, бот не воспринимает аудио/видео фалы, стикеры/картинки и не может на них отвечать. Пожулйста, выбери один из доступных пунктов меню. Инструкция как воспользоваться меню ниже')
...
def receive_postback(recipient_id, postback_body):
    if postback_body == 'StarT':
        send_message(recipient_id,
                     'Привет! Я могу помочь тебе разобраться с Мобильной платформой, а именно поделиться инструкциями как использовать приложение, узнать свежую информацию по мобильной платформе, котнаткы куратора твоего магазина и что делать в случае проблем с телефоном. На инструкции ниже показано как получить доступ к моему меню.')
        send_image_by_id(recipient_id, "735516596944436")
        return 'pass'
        ...
        send_message(recipient_id,
                     'Выбранная возможность, к сожалению, пока не реализована. Она появиться как только будет возможно 😉')
        return 'pass'
