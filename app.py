        ...
        for event in output['entry']:
            ...
            for message in messaging:
                ...
                if message.get('message'):
                    ...
                    if check_turn(recipient_id) == False:
                        send_message(recipient_id, 'Воспользуйся меню. Полная иснтрукция по ссылке: + link_help')
                    if check_turn(recipient_id) == True:
                        curent_action = get_curent_action(recipient_id)
                        if curent_action == 'Err':
                            send_message(recipient_id, 'Произошла ошибка. Попробуй воспользоваться меню.')
                        else:
                            curent_step = get_curent_step(recipient_id, curent_action)
                            check_dict = check_messege(curent_action, curent_step, text_message)
                            if check_dict['check']:
                                check_text_message = check_dict['check_value']
                            else:
                                'err'
...
def check_messege(curent_action, curent_step, text_message, recipient_id):
    if curent_action == 'NewPeopleRegistration':
        if curent_step == 'start_date':
            ...
            try:
                if datetime.datetime.strptime(text_message, '%d.%m.%Y') > datetime.datetime.strptime(start_date, '%d.%m.%Y'):
                    return {'check':True, 'check_value':text_message}
                else:
                    return {'check': False}
            except:
